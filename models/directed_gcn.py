...
class GraphConvolution_5part(nn.Module):

    def __init__(self,in_features,out_features,bias=False):
        super(GraphConvolution_5part, self).__init__()
        self.in_features = in_features
        self.out_features = out_features

        self.weight_center = Parameter(torch.FloatTensor(in_features,out_features))
        self.weight_farther = Parameter(torch.FloatTensor(in_features, out_features))
        self.weight_closer = Parameter(torch.FloatTensor(in_features, out_features))
        self.weight_symmetric = Parameter(torch.FloatTensor(in_features, out_features))
        self.weight_background = Parameter(torch.FloatTensor(in_features, out_features))
    ...
    def reset_parameters(self):
        ...
        torch.nn.init.xavier_uniform_(self.weight_center)
        torch.nn.init.xavier_uniform_(self.weight_farther)
        torch.nn.init.xavier_uniform_(self.weight_closer)
        torch.nn.init.xavier_uniform_(self.weight_symmetric)
        torch.nn.init.xavier_uniform_(self.weight_background)
        # if self.bias is not None:
        #     self.bias.data.uniform_(-stdv,stdv)

    def forward(self, input, adj, relu=False):
        support1 = torch.matmul(input, self.weight_center)
        support2 = torch.matmul(input, self.weight_closer)
        support3 = torch.matmul(input, self.weight_farther)
        support4 = torch.matmul(input, self.weight_symmetric)
        support5 = torch.matmul(input, self.weight_background)

        # print(support.size(),adj.size())
        output1 = torch.matmul(adj[0], support1)
        output2 = torch.matmul(adj[1], support2)
        output3 = torch.matmul(adj[2], support3)
        output4 = torch.matmul(adj[3], support4)
        output5 = torch.matmul(adj[4], support5)
